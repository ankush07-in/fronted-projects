...
function isCollide(_sarr) {
    // If you bump into yourself & snake is defined as {_sarr}
    for(let i = 1; i < snakeArr.length; i++) {
        if(_sarr[i].x === _sarr[0].x && _sarr[i].y === _sarr[0].y){
            return true;
        }
    }
    // If you bump into the wall
    if(_sarr[0].x >= 18 || _sarr[0].x <= 0 || _sarr[0].y >= 18 || _sarr[0].y <= 0){
        return true;
    }
}
function gameEngine(){
